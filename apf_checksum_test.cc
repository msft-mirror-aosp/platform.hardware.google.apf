...
TEST(ApfChecksumTest, CalcIPv4UDPChecksum) {
    ...
    *(uint16_t*) (ipv4_udp_pkt + 40) = htons(sizeof(ipv4_udp_pkt) - 20 - 14);
    uint8_t dscp = calculate_checksum_and_return_dscp(ipv4_udp_pkt, sizeof(ipv4_udp_pkt));
    EXPECT_EQ(dscp, 0);
...
TEST(ApfChecksumTest, CalcIPv6UDPChecksum) {
    ...
    // Set the UDP checksum to UDP payload size
    *(uint16_t*) (ipv6_udp_pkt + 60) = htons(sizeof(ipv6_udp_pkt) - 40 - 14);
    uint8_t dscp = calculate_checksum_and_return_dscp(ipv6_udp_pkt, sizeof(ipv6_udp_pkt));
    EXPECT_EQ(dscp, 0);
    // verify UDP checksum
...
TEST(ApfChecksumTest, CalcICMPv6Checksum) {
    ...
    *(uint16_t*) (ipv6_icmp6_pkt + 56) = htons(sizeof(ipv6_icmp6_pkt) - 40 - 14);
    uint8_t dscp = calculate_checksum_and_return_dscp(ipv6_icmp6_pkt, sizeof(ipv6_icmp6_pkt));
    EXPECT_EQ(dscp, 0);
